...
const physical=new Set(),touchPointers=new Map(),held={};let padInput={},padStart=false,ready=false,last=0,acc=0,ambient=0,muted=true,audio=null,toastEnd=0,dialogPaused=false,binding=null,storageFailed=false;
...
function tone(freq,duration=.1,type='sine',volume=.04,endFreq=0){if(muted||prefs.volume===0)return;try{audio??=new(window.AudioContext||window.webkitAudioContext)();if(audio.state==='suspended')void audio.resume();const o=audio.createOscillator(),g=audio.createGain();o.type=type;o.frequency.setValueAtTime(freq,audio.currentTime);if(endFreq)o.frequency.exponentialRampToValueAtTime(Math.max(20,endFreq),audio.currentTime+duration);g.gain.setValueAtTime(volume*prefs.volume/60,audio.currentTime);g.gain.exponentialRampToValueAtTime(.001,audio.currentTime+duration);o.connect(g);g.connect(audio.destination);o.start();o.stop(audio.currentTime+duration);}catch{}}
...
const game=new Game((type,data)=>{
 ...
 if(type==='slash')tone(data.kind==='heavy'?170:420+data.combo*90,data.kind==='heavy'?.26:.13,'sawtooth',.018,70);
 if(type==='dash')tone(160,.16,'triangle',.05,680);if(type==='jump')tone(170,.1,'sine',.025,390);
 if(type==='hit')tone(100,.09,'square',.04,40);if(type==='hurt')tone(75,.25,'sawtooth',.04,28);
 if(type==='shot')tone(490,.08,'triangle',.022,190);if(type==='kill')tone(180,.15,'triangle',.035,45);
...
function startView(){acc=0;$('mission').textContent=game.level.subtitle;showScreen('playing');canvas.focus({preventScroll:true});save();}
function pause(){if(game.mode==='playing'){game.mode='paused';showScreen('pause');}}
function resume(){if(game.mode==='paused'){game.mode='playing';showScreen('playing');canvas.focus({preventScroll:true});last=performance.now();acc=0;}}
...
$('sound').onclick=()=>{muted=!muted;$('sound').textContent=muted?'音 OFF':'音 ON';$('sound').setAttribute('aria-pressed',String(!muted));$('sound').setAttribute('aria-label',muted?'音をオンにする':'音をオフにする');tone(660,.15);};
...
$('difficulty').onchange=e=>{prefs.difficulty=e.target.value;savePrefs();};$('volume').oninput=e=>{prefs.volume=Number(e.target.value);savePrefs();};
...
window.addEventListener('blur',()=>{clearInput();pause();});window.addEventListener('gamepaddisconnected',()=>{padInput={};clearInput();pause();});
...
function formatTime(t){return String(Math.floor(t/60)).padStart(2,'0')+':'+String(Math.floor(t%60)).padStart(2,'0');}
  ...
  function sprite(f,x,y,dir=1,scale=.18,alpha=1,flash=false,breath=1){if(!f)return;ctx.save();ctx.translate(Math.round(x),Math.round(y));ctx.scale(dir*scale*f.scale,scale*f.scale*breath);ctx.globalAlpha=alpha;if(flash)ctx.filter='brightness(2.5)';ctx.drawImage(f.atlas,f.x,f.y,f.w,f.h,-f.ax,-f.ay,f.w,f.h);ctx.restore();}
  function heroFrame(){const p=game.player;if(p.dash>0)return 5;if(p.parry>0)return 0;if(p.attack>0)return p.attackKind==='heavy'||p.combo===3?7:6;if(!p.grounded)return 4;if(Math.abs(p.vx)>35)return[1,2,3,2][Math.floor(game.age*12)%4];return 0;}
  ...
  function enemy(e,cam){if(e.dead)return;const x=e.x+e.w/2-cam;if(x<-160||x>1120)return;
    ...
    const bob=e.type==='turret'?0:e.state==='idle'?Math.sin(e.age*8)*1.2:0;
    sprite(f,x,e.y+e.h+bob,-e.dir,scale,1,e.flash>0&&!reduced);
    if(e.state==='windup'){
  ...
  function boss(cam,t){const b=game.boss;if(b.dead||!b.active)return;const x=b.x+b.w/2-cam;
    ...
    sprite(f,x,b.y+b.h,-b.dir,scale,1,b.flash>0&&!reduced,1+Math.sin(t*2)*.005);
  ...
  function drawHero(cam,t){const p=game.player,x=p.x+p.w/2-cam;
    ...
    sprite(frames[heroFrame()],x,p.y+p.h,p.dir,.18,alpha,false,p.grounded&&Math.abs(p.vx)<20?1+Math.sin(t*3)*.006:1);
    if(p.attack>.07&&p.attack<(p.attackKind==='heavy'?.4:.26)){ctx.save();ctx.translate(x,p.y+28);ctx.scale(p.dir,1);
      const q=(.27-p.attack)/.185;ctx.strokeStyle=p.attackKind==='heavy'?'#ffe0a4':'#aaffec';ctx.lineWidth=p.attackKind==='heavy'?5:p.combo===3?4:2;ctx.shadowColor='#59fff0';ctx.shadowBlur=lowEffects?0:14;
      ctx.beginPath();ctx.ellipse(18,0,p.combo===3?84:76,40,-.2,-1.25+q*.7,1.15+q*.7);ctx.stroke();ctx.globalAlpha=.3;ctx.lineWidth=8;ctx.stroke();ctx.restore();}
  }
  function render(t){const cam=game.mode==='title'?300+Math.sin(t*.08)*70:game.camera;
    ...
    if(game.mode!=='title'){
      ...
      for(const q of game.projectiles){const x=q.x+q.w/2-cam,y=q.y+q.h/2,angle=Math.atan2(q.vy,q.vx),color=q.friendly?'#92ffdd':game.stageId===2?'#ffae67':'#ff659d';
      ...
      for(const fx of game.particles){ctx.globalAlpha=clamp(fx.life/fx.max,0,1);ctx.fillStyle=fx.color;ctx.fillRect(fx.x-cam,fx.y,fx.size+1,fx.size);}ctx.globalAlpha=1;
...
function loop(now){const dt=last?Math.min((now-last)/1000,.06):0;last=now;ambient+=dt;if(ready){pollGamepad();acc+=dt;while(acc>=1/60){game.update(1/60,held);acc-=1/60;}render(ambient);ui();}requestAnimationFrame(loop);}
